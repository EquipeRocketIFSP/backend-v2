...
public interface MedicamentoRespository extends JpaRepository<Medicamento, Long> {
    ...
    @Query("SELECT m FROM Medicamento m WHERE m.nome LIKE :search% OR m.nomeReferencia LIKE :search%")
    List<Medicamento> searchByNomeAndNomeReferencia(Pageable pageable, String search);

    @Query("SELECT COUNT(m.id) FROM Medicamento m WHERE m.nome LIKE :search% OR m.nomeReferencia LIKE :search%")
